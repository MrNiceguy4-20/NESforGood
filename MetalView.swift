...
final class Coordinator: NSObject, MTKViewDelegate {
    ...
    func configure(view: MTKView, emulator: EmulatorCore, scaleMode: ScaleMode, shaderMode: ShaderMode) {
        ...
        do {
            ...
            assertionFailure("Failed to build render pipeline: \(error)")
            return
        }
        ...
        guard let sampler = device.makeSamplerState(descriptor: sd) else {
            assertionFailure("Failed to create sampler state")
            return
        }
        ...
        guard let ub = device.makeBuffer(
            ...
            assertionFailure("Failed to create uniform buffer")
            return
        }
        self.ub = ub
