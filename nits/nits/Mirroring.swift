...
enum CartridgeError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .invalidROM:
            return "Invalid or unsupported NES ROM file."
        case .unsupportedMapper(let id):
            return "Unsupported NES mapper: \(id)."
        }
    }
}
...
final class Cartridge {
    ...
    init(data: Data) throws {
        ...
        default:
            // Fallback: try to treat unknown mappers as simple NROM-style carts so
            // test ROMs and homebrew with odd mapper IDs can still at least boot.
            // This will not be correct for real bank-switched carts, but avoids
            // outright load failure.
            print("Warning: unsupported mapper \(mapperID); using NROM-style fallback.")
            self.mapper = NROMMapper(prgROM: prgROM, chr: chr, prgRAM: prgRAM)
        }
    }
